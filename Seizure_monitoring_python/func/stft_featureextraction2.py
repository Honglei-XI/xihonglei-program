...
def getSpectral_STFT(data, fs):
    ...
    try:
        ...
        if data.ndim == 2:
            # 多通道数据，尝试处理第一个通道
            freqs, times, Pxx = stft(data[0], fs=fs, nperseg=nperseg, noverlap=noverlap)
            # 添加通道维度
            Pxx = Pxx[np.newaxis, :, :]
        else:
            # 单通道数据
            freqs, times, Pxx = stft(data, fs=fs, nperseg=nperseg, noverlap=noverlap)
    
    # 确保处理复数数据 - 计算幅度谱
    Pxx = np.abs(Pxx)
    
    print("Pxx 形状:", Pxx.shape)  # (n_channels, n_freqs, n_times) 或 (n_freqs, n_times)
    ...
    stft_data = (log_pxx - log_pxx.min()) / (log_pxx.max() - log_pxx.min())

    print("STFT结果的形状为 {}".format(stft_data.shape))
...
def create_results_folder():
    ...
    results_folder = os.path.join(base_dir, "results", f"stft_{timestamp}")
